...
func (b *deviceMapBuilder) updateDeviceMapWithProduct(oDevices DeviceMap) (DeviceMap, error) {
	...
	if err := b.VisitDevices(func(i int, d device.Device) error {
		...
		product = strings.ReplaceAll(strings.ReplaceAll(strings.ToLower(product), " ", "-"), "_", "-")

		for name, oDevice := range oDevices {
			for id, device := range oDevice {
				if strings.HasPrefix(id, uuid) {
					newName := strings.Replace(string(name), "gpu", fmt.Sprintf("gpu-%s", product), 1)
					devices.insert(spec.ResourceName(newName), device)
				}
			}
		}
		return nil
